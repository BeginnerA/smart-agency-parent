...
public final class PoiReflectorTools {

    private static final Map<Class<?>, PoiReflectorTools> CACHE_REFLECTOR = new ConcurrentHashMap<Class<?>, PoiReflectorTools>();

    private final Map<String, Method> getMethods = new HashMap<>();
    private final Map<String, Method> setMethods = new HashMap<>();
    private final Map<String, Method> enumMethods = new HashMap<>();
    private final Map<String, Field> publicField = new HashMap<>();
    private final List<Field> fieldList = new ArrayList<>();

    private Class<?> type;

    private PoiReflectorTools(Class<?> clazz) {
        this.type = clazz;
        addGetMethods(clazz);
        addFields(clazz);
        addSetMethods(clazz);
    ...
    public static PoiReflectorTools forClass(Class<?> clazz) {
        return new PoiReflectorTools(clazz);
    }

    public static PoiReflectorTools fromCache(Class<?> clazz) {
        if (!CACHE_REFLECTOR.containsKey(clazz)) {
            CACHE_REFLECTOR.put(clazz, new PoiReflectorTools(clazz));
        }
